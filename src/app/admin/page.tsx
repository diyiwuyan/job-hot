...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "hourly" | "modules" | "cities" | "users"
  >("overview");
    ...
    try {
      ...
      if (daily) {
        ...
        if (daily.length > 0 && compareDays.length === 0) {
          if (daily.length > 7) {
            setCompareDays([
              (daily[0] as DailyStat).day,
              (daily[7] as DailyStat).day,
            ]);
          } else if (daily.length > 1) {
            setCompareDays([
              (daily[0] as DailyStat).day,
              (daily[1] as DailyStat).day,
            ]);
          } else {
      ...
      const { data: cities } = await supabase.rpc("admin_city_stats", {
        days_back: 30,
      });
      if (cities) setCityStats(cities as CityStat[]);

      // User stats (distinct user_id counts)
      const now = new Date();
      const todayStr = now.toISOString().slice(0, 10);
      const weekAgoStr = new Date(now.getTime() - 7 * 86400000)
        .toISOString()
        .slice(0, 10);
      ...
      setUserStats({
        total_users: distinctCount(
          allUsersRes as { data: { user_id: string }[] | null },
        ),
        active_users_today: distinctCount(
          todayUsersRes as { data: { user_id: string }[] | null },
        ),
        active_users_7d: distinctCount(
          weekUsersRes as { data: { user_id: string }[] | null },
        ),
  ...
  const availableDays = Array.from(new Set(hourlyStats.map((h) => h.day)))
    .sort()
    .reverse();
    ...
    <div className="page admin-page">
      <div className="admin-header">
        <div>
          <h1>数据统计</h1>
          <p>
            {role === "super_admin" ? "超级管理员" : "管理员"} · {user.email}
          </p>
        </div>
      ...
      <div className="admin-tabs">
        {(["overview", "hourly", "modules", "cities", "users"] as const).map(
          (tab) => (
            <button
              key={tab}
              type="button"
      ...
      {loading ? (
        ...
        <>
                ...
                <div className="admin-stat-grid">
                  <div className="admin-stat-card">
                    <div className="admin-stat-value">
                      {dailyStats[0]?.unique_visitors ?? 0}
                    </div>
                    <div className="admin-stat-label">今日访客</div>
                  </div>
                  <div className="admin-stat-card">
                    <div className="admin-stat-value">
                      {dailyStats[0]?.page_views ?? 0}
                    </div>
                  ...
                  <div className="admin-stat-card">
                    <div className="admin-stat-value">
                      {dailyStats.length > 1 &&
                      dailyStats[1].unique_visitors > 0
                ...
                <table className="admin-table">
                  ...
                  <tbody>
                    {dailyStats.map((stat, i) => {
                      ...
                      const change =
                        prev && prev.unique_visitors > 0
                          ? (
                              (stat.unique_visitors / prev.unique_visitors -
                                1) *
                              100
                            ).toFixed(1)
                          : null;
              ...
              <div className="admin-day-picker">
                {availableDays.slice(0, 14).map((day, i) => (
                  <button
                    ...
                    style={
                      compareDays.includes(day)
                        ? {
                            borderColor:
                              CHART_COLORS[
                                compareDays.indexOf(day) % CHART_COLORS.length
                              ],
                            color:
                              CHART_COLORS[
                                compareDays.indexOf(day) % CHART_COLORS.length
          ...
          {activeTab === "cities" && (
            <div className="admin-section">
              <h3 className="admin-section-title">访客城市（近 30 天）</h3>
              <p className="admin-section-desc">
                仅展示粗粒度城市汇总；JOBHOT 不保存或展示原始 IP 地址。
              </p>
              {cityStats.length === 0 ? (
                <div className="admin-empty">暂未收集到可识别的城市数据</div>
              ) : (
                <div className="admin-table-wrap">
                  <table className="admin-table">
                    <thead>
                    ...
                    <tbody>
                      {cityStats.map((item, index) => (
                        <tr key={`${item.country_code}-${item.city}`}>
                          <td>{index + 1}</td>
                          <td>{item.city}</td>
          ...
          {activeTab === "users" && userStats && (
            <div className="admin-section">
              ...
              <div className="admin-stat-grid">
                ...
                <div className="admin-stat-card">
                  <div className="admin-stat-value">
                    {userStats.total_subscriptions}
                  </div>
                  <div className="admin-stat-label">订阅总数</div>
                </div>
                <div className="admin-stat-card">
                  <div className="admin-stat-value">
                    {userStats.active_subscriptions}
                  </div>
